...
model Boiler "Boiler"
  ...
  Modelica.Blocks.Sources.RealExpression realExpression1(y=if On > 0.1 then boi.T
         else THWSet)                                                    annotation (Placement(transformation(extent={{-20,-10},
            {-40,10}})));
equation
  ...
  connect(On, valCHW.y) annotation (Line(
      ...
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(conPI.On, valCHW.y) annotation (Line(
      points={{-80,34},{-96,34},{-96,-40},{-44,-40},{-44,-24},{70,-24},{70,-68}},
      color={0,0,127},
      pattern=LinePattern.Dash));

  connect(realExpression1.y, conPI.u_m) annotation (Line(
      points={{-41,0},{-54,0},{-68,0},{-68,14}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(conPI.y, boi.y) annotation (Line(
      points={{-57,26},{-8,26},{-8,-26}},
      color={0,0,127},
      pattern=LinePattern.Dash));
