...
model ChillerStage "Stage controller for chillers"
  ...
  Modelica.StateGraph.Transition One2Two(
    enableTimer=true,
    waitTime=tWai,
    condition=Status[1] > 0.9*1.2)
                   annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={-50,0})));
  Modelica.StateGraph.Transition Two2One(
    condition=(Status[1] < 0.3 and Status[2] < 0.3),
    enableTimer=true,
  ...
  Modelica.StateGraph.Transition Two2Three(
    enableTimer=true,
    waitTime=tWai,
    condition=Status[1] > 0.9*1.2 and Status[2] > 0.9*1.2)
                   annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={-50,-50})));
  Modelica.StateGraph.Transition Three2Two(
    waitTime=tWai,
    condition=(Status[1] < 0.5 and Status[2] < 0.5 and Status[3] < 0.5),
