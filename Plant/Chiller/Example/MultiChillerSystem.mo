within WaterSideSystem.Plant.Chiller.Example;
model MultiChillerSystem "Test model for MultiChillerSystem"
  import WaterSideSystem.Plant.Chiller;
  ...
  parameter Modelica.SIunits.Power P_nominal = 756E3/5.42
  ...
  parameter Real COP_nominal = 5.42 "Chiller COP";
  ...
  Modelica.Blocks.Sources.RealExpression mCW_flow(y=(OnA.y + OnB.y + OnC.y)*mCW_flow_nominal)
    annotation (Placement(transformation(extent={{-80,-70},{-60,-50}})));
  Modelica.Blocks.Sources.RealExpression mCHW_flow(y=(OnA.y + OnB.y + OnC.y)*mCHW_flow_nominal)
    annotation (Placement(transformation(extent={{80,2},{60,22}})));
equation

  connect(OnC.y, mulChiSys.On[1])
    annotation (Line(
      points={{59,80},{-40,80},{-40,-5.4},{-10.9,-5.4}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(OnB.y, mulChiSys.On[2]) annotation (Line(
      points={{-59,80},{-46,80},{-46,-6},{-10.9,-6}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(OnA.y, mulChiSys.On[3])
    annotation (Line(
      points={{-59,40},{-52,40},{-52,-6.6},{-10.9,-6.6}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
