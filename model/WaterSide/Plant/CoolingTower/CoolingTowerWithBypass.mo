...
model CoolingTowerWithBypass
  ...
  MultiCoolingTowerSystem mulCooTowSys(
    ...
    eta=eta) annotation (Placement(transformation(extent={{-40,-40},{-20,-20}})));
  BaseClasses.Components.Bypass byp(
    redeclare package MediumCW = MediumCW,
    m_flow_nominal=3*mCW_flow_nominal,
    dPByp_nominal=dPByp_nominal) annotation (Placement(transformation(extent={{0,-20},{20,0}})));
  Modelica.Blocks.Interfaces.RealInput On[3] "On signal"
    annotation (Placement(transformation(extent={{-118,51},{-100,69}})));
  BaseClasses.Control.BypassControl bypCon(TSet=TSet)
