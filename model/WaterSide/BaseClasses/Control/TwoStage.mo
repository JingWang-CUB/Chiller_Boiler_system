...
model TwoStage "Stage controller for pumps"
  ...
  Modelica.StateGraph.StepWithSignal OneOn(       nOut=2, nIn=3)
  ...
  Modelica.StateGraph.Transition Two2One(
    enableTimer=true,
    waitTime=tWai,
    condition=(Status[1] < 0.6 and Status[2] < 0.6 and TwoOn.active))
                               annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={54,6})));
  Modelica.StateGraph.Transition One2Off(condition=not On,
    waitTime=1,
    enableTimer=false)                                     annotation (
...
equation
  ...
  connect(One2Two.outPort, TwoOn.inPort[1]) annotation (Line(points={{-50,-21.5},
          {-50,-40},{0,-40},{0,-44},{-0.5,-44},{-0.5,-49}},
                                         color={0,0,0}));
  connect(Two2One.outPort, OneOn.inPort[2]) annotation (Line(points={{54,7.5},{54,
          7.5},{54,32},{7.21645e-016,32},{7.21645e-016,29}},
                                              color={0,0,0}));
  ...
  connect(OneOn.outPort[2], One2Off.inPort) annotation (Line(points={{0.25,7.5},
          {2,7.5},{2,-10},{30,-10},{30,28},{52,28},{52,44}},
                                                         color={0,0,0}));
  connect(Two2One.inPort, TwoOn.outPort[1]) annotation (Line(points={{54,2},{54,
          -92},{-0.25,-92},{-0.25,-70.5}},
                                         color={0,0,0}));
  ...
  connect(Two2OneOff.inPort, TwoOn.outPort[2]) annotation (Line(points={{84,12},
          {86,12},{86,-18},{90,-18},{90,-86},{0.25,-86},{0.25,-70.5}},   color={
          0,0,0}));
  connect(Two2OneOff.outPort, OneOn.inPort[3]) annotation (Line(points={{84,17.5},
          {84,17.5},{84,38},{0.666667,38},{0.666667,29}}, color={0,0,0}));
